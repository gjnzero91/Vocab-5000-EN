...
<<<<<<< HEAD
  async function loadStateAndData() {
    ...
    if (user) {
      console.log("User signed in. Loading data from Firestore...");
      await loadUserDataFromFirestore(user.uid);
    }
    const state = getBookState(currentBookKey);
    if (!state.words || state.words.length === 0) {
      console.log("No book data found. Initializing from Google Sheets...");
      await initializeBookData(currentBookKey);
    } else {
      console.log("Book data already exists. Skipping initialization from Google Sheets.");
    }
    const updatedBookState = getBookState(currentBookKey);
    if (shuffleToggle) {
      shuffleToggle.classList.toggle('active', updatedBookState.shuffleMode);
    }
    if (countdownElement) {
    countdownElement.textContent = updatedBookState.countdownTime;
    }
    loadBook();
  }
=======
